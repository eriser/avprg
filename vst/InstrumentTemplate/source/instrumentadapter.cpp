...
void InstrumentAdapter::processInputParameter (ProcessData& data)
{
...
ParamValue InstrumentAdapter::getPlainValue(ParamID paramId){
	Parameter* parameter = parameters.getParameter(paramId);
	ParamValue normalizedValue = parameter->getNormalized();
	return parameter->toPlain(normalizedValue);
}
bool InstrumentAdapter::hasInputParameterChanged (ProcessData& data, ParamID paramId)
{
    if (data.inputParameterChanges)	{
        int32 paramChangeCount = data.inputParameterChanges->getParameterCount ();
        for (int32 index = 0; index < paramChangeCount; index++){
...
ParamValue InstrumentAdapter::getInputParameterChange (ProcessData& data, ParamID paramId)
{
    if (data.inputParameterChanges)	{
        int32 paramChangeCount = data.inputParameterChanges->getParameterCount ();
        for (int32 index = 0; index < paramChangeCount; index++){
...
void InstrumentAdapter::setOutputParameterChange(ProcessData &data, ParamID paramId, ParamValue value){
...
//-----------------------------------------------------------------------------
tresult PLUGIN_API InstrumentAdapter::setActive (TBool state)
{
...
//-----------------------------------------------------------------------------
tresult PLUGIN_API InstrumentAdapter::initialize (FUnknown* context)
{
	tresult result = SingleComponentEffect::initialize (context);
	if (result != kResultOk)
		return result;
	// VST Instrument
	// configure: no audio input but midi input 
...
//-----------------------------------------------------------------------------
tresult PLUGIN_API InstrumentAdapter::setBusArrangements (SpeakerArrangement* inputs, int32 numIns, SpeakerArrangement* outputs, int32 numOuts)
{
	// see againsimple.cpp
...
//------------------------------------------------------------------------
IPlugView* PLUGIN_API InstrumentAdapter::createView (const char* name)
{
...
//------------------------------------------------------------------------
tresult PLUGIN_API InstrumentAdapter::terminate  ()
{
...
//------------------------------------------------------------------------
void InstrumentAdapter::addDependentView (IParamUpdater* view)
{
	viewsArray.add (view);
}

//------------------------------------------------------------------------
void InstrumentAdapter::removeDependentView (IParamUpdater* view)
{
...
//------------------------------------------------------------------------
void InstrumentAdapter::editorAttached (EditorView* editor)
{
...
//------------------------------------------------------------------------
void InstrumentAdapter::editorRemoved (EditorView* editor)
{
	IParamUpdater* view = dynamic_cast<IParamUpdater*> (editor);
	if (view)
	{
...
//------------------------------------------------------------------------
tresult PLUGIN_API InstrumentAdapter::setParamNormalized (ParamID tag, ParamValue value)
{
...
{
	return Steinberg::Vst::Vst2Wrapper::create (GetPluginFactory (), Steinberg::Vst::InstrumentUID, 'GnV2', audioMaster);
}
...
BEGIN_FACTORY_DEF ("Steinberg Media Technologies", 
	...
	DEF_CLASS2 (INLINE_UID_FROM_FUID(InstrumentUID),
				...
				Steinberg::Vst::Instrument::createInstance)// function pointer called when this component should be instanciated
